...
namespace MovieDatabase.DataAccess.Repository
{
    public class MovieRepository : IMovieRepository
    {
        ...
        public string GetGenreByMovieId(int movieId)
        {
            return _context.Movies.FirstOrDefault(m => m.Id == movieId).Genre;
        }

        public List<Movie> GetMoviesByGenre(string genre)
        {
            return _context.Movies.Where(m => m.Genre == genre).ToList();
        }
    }
}